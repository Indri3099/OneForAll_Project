...
public class runner {
    public static void main(String[] args) {
        ...
        while (scanner.hasNextLine()) {
            ...
            PhraseReduction p = null;
            try {
                p = parser.analyze(command, game.getCurrentRoom(), game.getCommandList(), game.getMainCharacter());
                if (p.getCommand() != null && p.getCommand().getType() == CommandType.END) {
                    ...
                    try {
                        ...
                        if (game.getCurrentRoom().getEventHandler() != null) {
                            game.getCurrentRoom().getEventHandler().completeEvent(game, System.out);
                        }
                    } catch (Exception ex) {
